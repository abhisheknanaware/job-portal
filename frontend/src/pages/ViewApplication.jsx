...
const ViewApplication = () => {
  ...
  return applicants ? applicants.length ===0?(
    <div className='flex items-center justify-center h-[70vh]'>
      <p className='text-xl sm:text-2xl'>No Applicantions Found</p>
    </div>
  ): (
              ...
              <tr key={idx} className='text-gray-700'>
                ...
                <td className='py-2 px-4 border-b'>
                  <a href={applicant.userId.resume} target='_blank'>
                    Resume <img className='bg-blue-50 text-blue-400 px-3 py-1 rounded inline-flex gap-2 items-center' src={assets.resume_download_icon} alt=""/>
                  </a>
                </td>
                <td className='py-2 px-4 border-b relative'>
                  ...
                  (<div className='relative inline-block text-left group'>
                    ...
                    <div className='z-10 hidden absolute right-0 md:left-0 -top-5 mt-2 w-32 bg-white border border-gray-200 rounded shadow group-hover:block'>
                      <button onClick={()=>statusHandler(applicant._id,'Accepted')} className='block w-full text-left px-4 py-2 text-blue-500 hover:bg-gray-100'>Accept</button>
                      <button onClick={()=>statusHandler(applicant._id,'Rejected')} className='block w-full text-left px-4 py-2 text-red-500 hover:bg-gray-100'>Reject</button>
                    </div>
                  </div>)
                  :(
